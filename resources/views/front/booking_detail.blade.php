            ...
            <div class="flex items-center justify-between rounded-3xl p-[6px] pr-[14px] bg-white overflow-hidden">
                <div class="flex items-center gap-[14px]">
                    <div class="flex w-[90px] h-[90px] shrink-0 rounded-3xl bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ Storage::url($bookingTransaction->ticket->thumbnail) }}" class="w-full h-full object-cover" alt="thumbnail">
                    </div>
                    <div class="flex flex-col gap-[6px]">
                        <h3 class="font-semibold">Getaway Balloon</h3>
                        <div class="flex items-center gap-1">
                            <img src="{{ asset('assets/images/icons/location.svg') }}" class="w-[18px] h-[18px]" alt="icon">
                            <p class="font-semibold text-xs leading-[18px]">{{ $bookingTransaction->ticket->name }}</p>
                        </div>
                        <p class="font-bold text-sm leading-[21px] text-[#F97316]">Rp {{ number_format($bookingTransaction->ticket->price, '0', ',', '.') }}</p>
                    </div>
                </div>
                ...
                <div class="relative flex flex-col p-5 pb-[30px] gap-6">
                    ...
                    <div class="flex flex-col gap-[14px] shrink-0 h-full">
                        <div class="flex items-center justify-between">
                            ...
                            <p class="font-bold text-xl leading-[30px]">{{ $bookingTransaction->booking_trx_id }}</p>
                        </div>
                        <div class="flex items-center justify-between">
                            <p class="font-bold text-sm leading-[21px]">Started At</p>
                            <p class="font-bold text-sm leading-[21px] text-[#FF1927]">{{ $bookingTransaction->started_at->format('d M Y') }}</p>
                        </div>
                        <div class="flex items-center justify-between">
                            <p class="font-bold text-sm leading-[21px]">Total People</p>
                            <p class="font-bold text-sm leading-[21px]">{{ $bookingTransaction->total_participant }} Participant</p>
                        </div>
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <p class="font-bold text-[22px] leading-[33px] text-[#F97316]">Rp {{ number_format($bookingTransaction->total_amount, '0', ',', '.') }}</p>
                        </div>
                        <div class="flex items-center justify-between">
                            ...
                            @else
                                <p class="w-fit rounded-full p-[6px_12px] bg-[#07B704] font-bold text-xs leading-[18px] text-white">PENDING</p>
                            @endif
                        </div>
                    </div>
                    ...
                    @if ($bookingTransaction->is_paid)
                        <div class="flex items-center rounded-[20px] p-[10px] gap-[10px] bg-[#F8F8F9]">
                            ...
                            <p class="leading-[28px]">Gunakan kode <span class="font-bold">{{ $bookingTransaction->booking_trx_id }}</span> ketika menukarkan dengan tiket asli</p>
                        </div>
                    @else
                        <div class="flex items-center rounded-[20px] p-[10px] gap-[10px] bg-[#F8F8F9]">
                            <img src="{{ asset('assets/images/icons/ticket-star-black.svg') }}" class="w-8 h-8" alt="icon">
                            <p class="leading-[28px]">Pembayaran Anda saat ini  pending, tiket belum tersedia.</p>
                        </div>
                    @endif
